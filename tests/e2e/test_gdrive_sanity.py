...
from evernote_to_gdrive.auth import resolve_secrets_dir, token_path, get_services
...
def gdrive_migration(request):
    if not token_path(resolve_secrets_dir(None)).exists():
...
def test_gdrive_sanity(gdrive_migration: tuple) -> None:
    drive, records = gdrive_migration
    ...
    # Verify inter-note link from source → target is preserved as a Google Docs URL
    ...
    # Verify duplicate note produced two copies of each output file on Drive
