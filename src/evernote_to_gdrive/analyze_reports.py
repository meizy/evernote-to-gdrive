...
from .classifier import classify, _safe_name, _SKIP_MIME
...
def list_attachments(notes: Iterable[Note]) -> None:
    rows: list[tuple[str, str, int, int, int, bool]] = []
    for note in notes:
        if not note.attachments:
            continue
        images = sum(1 for a in note.attachments if a.mime.startswith("image/"))
        pdfs = sum(1 for a in note.attachments if a.mime == "application/pdf")
        other = len(note.attachments) - images - pdfs
        nb = format_notebook(note.stack, note.notebook)
        rows.append((nb, note.title, images, pdfs, other, note.source_url is not None))
    ...
    for nb, title, images, pdfs, other, web_clip in sorted(rows):
        t.add_row(nb, rtl_display(title), str(images), str(pdfs), str(other), "Y" if web_clip else "")
    console.print()
    console.print(t)
    console.print(f"\n[dim]{len(rows)} note(s) with attachments.")
