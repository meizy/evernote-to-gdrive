...
def _run_migration(filtered: list[NotebookInfo], options: MigrationOptions, writer, seen: dict, records: list,
    ...
    show_progress = not (options.verbose or options.debug)
    ctx = _migration_progress() if show_progress else None
    ...
    with (ctx if ctx else _nullcontext()) as progress:
        ...
        for info in filtered:
            ...
            for note in parse_enex(info.path, stack=info.stack):
                ...
                if progress:
                    progress.update(task, description=f"[cyan]{format_notebook(note.stack, note.notebook)}[/] / {rtl_display(note.title[:40])}")
                t0 = time.monotonic()
                ...
                if _log.isEnabledFor(logging.DEBUG):
                    nb_label_plain = f"{format_notebook(note.stack, note.notebook)} / {rtl_display(note.title)}"
                    if record.status == MigrationStatus.SKIPPED:
                        _log.debug("%s - skipped", nb_label_plain)
                    elif gdrive:
                        _log.debug("%s (%.1fs)", nb_label_plain, elapsed)
                    else:
                        _log.debug("%s", nb_label_plain)
                if verbose:
                    nb_count += 1
                    label = f"[cyan]{format_notebook(note.stack, note.notebook)}[/] / {rtl_display(note.title)}"
                    if record.status == MigrationStatus.SKIPPED:
            ...
            _log.debug("%s: %.1fs, %.1fMB", format_notebook(info.stack, info.notebook), nb_elapsed, nb_mb)
            if verbose:
                console.print(f"  [dim]{format_notebook(info.stack, info.notebook)}: {nb_count} notes, {nb_elapsed:.1f}s, {nb_mb:.1f}MB")
                if gdrive:
...
def _rewrite_deferred_interlinks(writer, deferred: list[DeferredInterlinkNote], title_to_drive_file: dict[str, tuple[str, bool]],
                        duplicate_titles: set[str], verbose: bool = False, debug: bool = False) -> None:
    total_resolved = total_unresolved = 0
    console.print()
    console.rule("pass 2")
    show_progress = not (verbose or debug)
    ctx = _migration_progress() if show_progress else _nullcontext()
    with ctx as progress:
        task = progress.add_task("Rewriting inter-note links", total=len(deferred)) if progress else None
        ...
        if progress:
            progress.update(task, description="[green]complete[/]")
    _log.info("Links rewritten: %d resolved, %d unresolved", total_resolved, total_unresolved)
...
def run_migration(input_path: Path, options: MigrationOptions) -> list[MigrationRecord]:
    ...
    try:
        total = 0 if (options.verbose or options.debug) else count_notes(filtered)
...
def _print_summary(records: list[MigrationRecord], seen_stacks: set[str], seen_notebooks: set[str], is_gdrive: bool) -> None:
    ...
    if is_gdrive:
        ...
        _log.debug("  Uploaded:  ~%.1f MB (estimate)", mb)
    console.print()
