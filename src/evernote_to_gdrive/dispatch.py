...
def migrate_note(
    ...
    if _log.isEnabledFor(logging.DEBUG):
        mime_counts = Counter(a.mime for a in classified.attachments)
        mime_summary = ", ".join(
            f"{n}x {m}" for m, n in sorted(mime_counts.items(), key=lambda kv: (-kv[1], kv[0]))
        ) or "no attachments"
        _log.debug(
            "note %s: %s, %s",
            rtl_display(note.title),
            "has text" if classified.plain_text else "no text",
            mime_summary,
        )
